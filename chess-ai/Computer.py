        ...
        for j in range(8):
            ...
            if isinstance(piece, ChessPiece) and ((maximising and piece.color != board.get_player_color()) or
                ...
                for move in piece.get_moves(board):
                    ...
                    score = board.evaluate(True) if board.get_player_color() == "white" else board.evaluate(False)
...
def minimax2(board, depth,max_player, alpha, beta, save_move=False,):
    global positions_evaluated
    if depth == 0:
        return [[0,0,0]]
    data = []
    ...
    if max_player:
        ...
        for score, piece, move in sorted_moves:
            board.make_move(piece, move[0], move[1], True)
            mini = minimax2(board, depth - 1, False, alpha, beta)
            enemy_data = mini[0][2]
            evaluation = 0
            if board.get_player_color() == "white":
                evaluation += board.evaluate(False)
        ...
        for score, piece, move in sorted_moves:
            board.make_move(piece, move[0], move[1], True)
            mini = minimax2(board, depth-1,True, alpha, beta)
